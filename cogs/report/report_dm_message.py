...
class ReportDmMessageCog(commands.Cog):
    ...
    async def report(self, inter: discord.Interaction, message: discord.Message):
        """DMのメッセージをサーバー運営に通報します。"""
        if message.author == inter.user:
            return await inter.response.send_message("自分自身のメッセージを通報することはできません。", ephemeral=True)
        ...
        e.set_footer(text="⚠️まだ通報は完了していません⚠️\nサーバー内で</report:1221696906770649090>コマンドを使用して通報を完了してください。")
...
async def setup(bot):
    await bot.add_cog(ReportDmMessageCog(bot))